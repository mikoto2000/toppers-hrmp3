 ...
 *  の責任を負わない．
 * 
 *  $Id: task.c 729 2020-01-02 00:00:30Z ertl-hiro $
 */
...
/*
 *  実行すべきタスクの更新（自プロセッサ向け）
 */
#ifdef TOPPERS_myschedtsk

void
update_my_schedtsk(PCB *p_my_pcb)
{
	if (exist_runnable(p_my_pcb->p_schedcb_kernel)) {
		if (exist_runnable(p_my_pcb->p_twdsched)
					&& p_my_pcb->p_twdsched->p_predtsk->priority
						< p_my_pcb->p_schedcb_kernel->p_predtsk->priority) {
			p_my_pcb->p_schedtsk = p_my_pcb->p_twdsched->p_predtsk;
		}
		else {
			p_my_pcb->p_schedtsk = p_my_pcb->p_schedcb_kernel->p_predtsk;
		}
	}
	else {
		if (exist_runnable(p_my_pcb->p_twdsched)) {
			...
			if (p_my_pcb->p_idlesched != NULL) {
				p_my_pcb->p_schedtsk = p_my_pcb->p_idlesched->p_predtsk;
			}
			else {
				p_my_pcb->p_schedtsk = NULL;
			}
...
void
update_schedtsk(PCB *p_my_pcb, PCB *p_pcb)
{
	TCB		*p_prev_schedtsk;
