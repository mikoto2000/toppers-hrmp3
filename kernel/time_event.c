 ...
 *  の責任を負わない．
 * 
 *  $Id: time_event.c 393 2019-07-10 06:18:39Z ertl-hiro $
 */
 ...
 */
#define p_top_tmevtn_kernel(p_pcb)	p_top_tmevtn(p_pcb->p_tmevt_heap_kernel)
#define top_evttim_kernel(p_pcb)	top_evttim(p_pcb->p_tmevt_heap_kernel)
#define p_last_tmevtn_kernel(p_pcb)	p_last_tmevtn(p_pcb->p_tmevt_heap_kernel)
...
initialize_tmevt(PCB *p_my_pcb)
{
	...
	uint_t	i;
	TMEVTN	*p_tmevt_heap_kernel, *p_tmevt_heap_idle, *p_tmevt_heap;
	...
	p_my_pcb->in_signal_time = false;				/*［ASPD1033］*/

	p_tmevt_heap_kernel = dominib_kernel.p_tmevt_heap[my_prcidx];
	if (p_tmevt_heap_kernel != NULL) {
		p_last_tmevtn(p_tmevt_heap_kernel) = p_tmevt_heap_kernel;
	}
	p_my_pcb->p_tmevt_heap_kernel = p_tmevt_heap_kernel;

	p_tmevt_heap_idle = p_tmevt_heap_idle_table[my_prcidx];
	if (p_tmevt_heap_idle != NULL) {
		p_last_tmevtn(p_tmevt_heap_idle) = p_tmevt_heap_idle;
	}
	p_my_pcb->p_tmevt_heap_idle = p_tmevt_heap_idle;

	for (i = 0; i < tnum_udom; i++) {
		p_tmevt_heap = dominib_table[i].p_tmevt_heap[my_prcidx];
		if (p_tmevt_heap != NULL && p_tmevt_heap != p_tmevt_heap_kernel
...
set_hrt_event(PCB *p_pcb)
{
	...
	if (p_last_tmevtn_kernel(p_pcb) < p_top_tmevtn_kernel(p_pcb)) {
		/* 
		 *  タイムイベントがない場合
		 */
	...
	else if (EVTTIM_LE(top_evttim_kernel(p_pcb), current_evttim)) {
		/*
		...
		hrtcnt = (HRTCNT)(top_evttim_kernel(p_pcb) - current_evttim);
#ifdef USE_64BIT_HRTCNT
...
void
tmevtb_enqueue(TMEVTB *p_tmevtb, TMEVTN *p_tmevt_heap, PCB *p_pcb)
{
	/*
	 ...
	 */
	if (p_tmevt_heap == p_pcb->p_tmevt_heap_kernel && !(p_pcb->in_signal_time)
...
void
tmevtb_dequeue(TMEVTB *p_tmevtb, TMEVTN *p_tmevt_heap, PCB *p_pcb)
{
	TMEVTN	*p_tmevtn;
...
check_adjtim(int32_t adjtim, PCB *p_pcb)
{
	if (adjtim > 0) {							/*［NGKI3588］*/
		return(p_last_tmevtn_kernel(p_pcb) >= p_top_tmevtn_kernel(p_pcb)
			&& EVTTIM_LE(top_evttim_kernel(p_pcb) + TMAX_ADJTIM, current_evttim));
	}
...
signal_time(void)
{
	...
	lock_cpu();
	acquire_glock();
	p_my_pcb->in_signal_time = true;						/*［ASPD1033］*/
	...
	do {
		 ...
		 *  ［ASPD1018］［ASPD1019］．
		 */
		while (p_last_tmevtn_kernel(p_my_pcb) >= p_top_tmevtn_kernel(p_my_pcb)
					&& EVTTIM_LE(top_evttim_kernel(p_my_pcb), current_evttim)) {
			p_tmevtb = tmevtb_delete_top(p_my_pcb->p_tmevt_heap_kernel);
			(*(p_tmevtb->callback))(p_tmevtb->arg);
			callflag = true;
#ifndef TOPPERS_OMIT_SYSLOG
			nocall += 1;
